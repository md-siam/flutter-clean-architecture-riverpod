...
- **`lib/shared/`** — presentation infra used by *more than one* feature, with no business logic of its own: `app/` (root `App` widget), `route/` (auto_route config + generated router), `theme/`, `widgets/` (reusable, feature-agnostic components), `locale/`.
  - **`base/`** — each feature's `domain/entity/*.dart` and data `models/*.dart` file is `part of` one of `shared/base/base_entity.dart` / `base_response.dart` / `base_request.dart` rather than a standalone freezed library. This means freezed/json_serializable emit one `base_entity.freezed.dart` / `base_response.{freezed,g}.dart` / `base_request.{freezed,g}.dart` in `shared/base/`, instead of a generated-file pair scattered into every feature. A feature's entity/model file itself still lives under that feature's own `domain/entity/` or `data/models/` — only the generated output is centralized. Importing `LoginEntity`/`UserEntity` etc. anywhere means importing the relevant `shared/base/base_*.dart` file, not the feature file directly (Dart resolves `part of` symbols through the library file). `create_feature.sh` wires new entities/models into these aggregators automatically.
